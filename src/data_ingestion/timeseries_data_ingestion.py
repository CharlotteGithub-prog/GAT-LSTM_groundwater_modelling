...
# def _get_bbox_and_polygon_old(catchment, shape_filepath, required_crs):
#     # Derive full path
...
# def _estimate_cell_area_old(lat, lon, dlat, dlon, g):
    """
...
# def _compute_weighted_sum_aet_old(full_aet_da):
...
#             area_array[i, j] = _estimate_cell_area_old(lat, lon, lat_resolution, lon_resolution, g)
...
# def _combine_and_aggregate_daily_aet_old(all_daily_aet_dataarrays, processed_output_dir,start_year, end_year):
...
#     catchment_sum_aet = _compute_weighted_sum_aet_old(full_aet_da)
...
# def _extract_zip_file_old(zip_filename, raw_output_dir, year, month):
...
# def _apply_catchment_mask_old(catchment_polygon, ds, daily_mm):
#     """
...
# def _call_era5_api_old(start_year, end_year, start_date, end_date, total_months, raw_output_dir, north,
...
#                 grib_filename = _extract_zip_file_old(zip_filename, raw_output_dir, year, month)
...
# def _resample_by_time_and_step_old(ds):
...
# def _save_aet_graph_old(aet_csv_path, aet_fig_path):
...
# def _process_local_grib_files_old(raw_output_dir, catchment_polygon, all_daily_aet_dataarrays):
...
#                 daily_mm = _resample_by_time_and_step_old(ds)
...
#                 masked = _apply_catchment_mask_old(catchment_polygon, ds, daily_mm_sliced)
...
# def load_aet_data_old(catchment: str, shape_filepath: float, required_crs: int, cdsapi_path: str,
...
#     north, south, east, west, catchment_polygon = _get_bbox_and_polygon_old(
...
#         _call_era5_api_old(start_year, end_year, start_date, end_date, total_months,
#                       raw_output_dir, north, west, south, east, c, catchment_polygon)

#     # Always process data
#     _process_local_grib_files_old(raw_output_dir, catchment_polygon, all_daily_aet_dataarrays)
    
#     if all_daily_aet_dataarrays:
        
#         # --- Aggregate to daily values for catchment ---
#         catchment_sum_aet = _combine_and_aggregate_daily_aet_old(
#             all_daily_aet_dataarrays, processed_output_dir,start_year, end_year)
        
#         # --- Save time series data to results ---
#         _save_aet_graph_old(aet_csv_path, aet_fig_path)
...
def _compute_weighted_aggregation(full_da, feat_name, aggregation_type):
    # Estimate the grid spacing in degrees assuming uniform spacing (true for ERA5-Land)
    lat_resolution = abs(full_da.latitude[1] - full_da.latitude[0]).item()
    lon_resolution = abs(full_da.longitude[1] - full_da.longitude[0]).item()
    ...
    # For 'mean' need area-weighted average (sum of value * area) / (sum of area)
        ...
        catchment_aggregated_data = weighted_sum / area_da.sum()
...
def _resample_by_time_and_step(ds, era5_feat, aggregation_type):
    ...
    if era5_feat == 'e':  # AET
        ...
        transformed_data = feat_flat - 273.15  # Conver from ºK to ºC
    elif era5_feat == 'sp':  # surface pressure
        transformed_data = feat_flat  # Previoudly: Pa to hPa
    else:
...
def load_era5_land_data(catchment: str, shape_filepath: float, required_crs: int, cdsapi_path: str,
    ...
    if run_era5_land_api:
        ...
        _call_era5_api(start_year, end_year, start_date, end_date, total_months, raw_output_dir,
                       north, west, south, east, c, era5_feat, era5_long, feat_name, aggregation_type)
